Shader "TK/PBR/Model/PBRModelShader"
{
Properties
    {
        [Header(BaseColor)]
        ...
        [Header(Matcap)]
        [Toggle(_MATCAP)]_MATCAP("_MATCAP?", Float) = 1
        _MatcapTex("MatCapTex", 2D) = "white" { }
        _MatcapPower("MatcapPower", Range(0.0, 1.0)) = 0.5

        [Header(Rim)]
        [Toggle(_RIM)]_RIM("_RIM?", Float) = 1
    ...
    float4 GetShadowPositionHClip(VaryingsPBR input)
    {
    ...
    void UserGeometryDataOutputFunction(AttributesPBR IN, inout UserGeometryOutputData outputData, bool isExtraCustomPass);

    UserGeometryOutputData BuildUserGeometryOutputData(AttributesPBR IN, bool isExtraCustomPass = false)
    {
    ...
    VaryingsPBR VertAllWork(AttributesPBR IN, bool shouldApplyShadowBias = false, bool isExtraCustomPass = false)
    {
    ...
    VaryingsPBR vertUniversalForward(AttributesPBR IN)
    {
        return VertAllWork(IN);
    }
    VaryingsPBR vertShadowCaster(AttributesPBR IN)
    {
        return VertAllWork(IN, true, false);
    }
    VaryingsPBR vertExtraCustomPass(AttributesPBR IN)
    {
    ...
    void UserSurfaceOutputDataFunction(VaryingsPBR IN, inout UserSurfaceOutputData surfaceData, bool isExtraCustomPass);

    UserSurfaceOutputData BuildUserSurfaceOutputData(VaryingsPBR IN, bool isExtraCustomPass)
    {
    ...
    half4 CalculateSurfaceFinalResultColor(VaryingsPBR IN, UserSurfaceOutputData surfaceData, LightingsData lightingData);

    void FinalPostProcessFrag(VaryingsPBR IN, UserSurfaceOutputData surfaceData, LightingsData lightingData, inout half4 inputColor)
    {
#if _IsSelected
    ...
    half4 fragAllWork(VaryingsPBR IN, bool shouldOnlyDoAlphaClipAndEarlyExit = false, bool isExtraCustomPass = false)
    {
...
#if _MATCAP
        float2 uv = TRANSFORM_TEX(IN.uv, _MatcapTex);
        float3 matCap = SAMPLE_TEXTURE2D(_MatcapTex, sampler_MatcapTex, uv);
        finalColor.rgb += matCap * _MatcapPower;
#endif

#if _RIM
        float2 uv2 = TRANSFORM_TEX(IN.uv, _RimTex);
        float3 rim = SAMPLE_TEXTURE2D(_RimTex, sampler_RimTex, uv2) * _RimPower;
        float rimp = 1 - saturate(dot(viewDirectionWS, N)) * _RimPower;
        finalColor.rgb += rim.rgb * pow(rimp, 2.5);
#endif
    return finalColor;
    }
    half4 fragUniversalForward(VaryingsPBR IN) : SV_Target
    {
        return fragAllWork(IN);
    }
    half4 fragDoAlphaClipOnlyAndEarlyExit(VaryingsPBR IN) : SV_Target
    {
        return fragAllWork(IN, true);
    }
    half4 fragExtraCustomPass(VaryingsPBR IN) : SV_Target
    {
...
# ifndef _SPECULARHIGHLIGHTS_OFF
    ...
    void UserGeometryDataOutputFunction(AttributesPBR IN, inout UserGeometryOutputData geometryOutputData, bool isExtraCustomPass)
    {
    ...
    SubShader
    {
        ...
        //UnityLitShaderのDepthNormalパス　SSAOでDepthに映らなくなるバグを解決
        //---------------
        Pass
        {
            Name "DepthOnly"
